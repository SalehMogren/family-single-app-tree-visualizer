...
export const RelationshipManager: React.FC<RelationshipManagerProps> = ({
  ...
  const quickAddActions = [
    {
      ...
      disabled: selectedPerson.parents && selectedPerson.parents.length >= 2,
      disabledReason: "Maximum 2 parents allowed",
    },
    {
  ...
  const renderModifySection = () => {
    const currentRelationships = {
      parents: selectedPerson.parents?.map(id => allData[id]).filter(Boolean) || [],
      spouses: selectedPerson.spouses?.map(id => allData[id]).filter(Boolean) || [],
      children: selectedPerson.children?.map(id => allData[id]).filter(Boolean) || [],
    ...
    return (
      <div className='space-y-4'>
        ...
        {Object.entries(currentRelationships).map(([relationshipType, people]) => (
          <Card key={relationshipType} className='p-4'>
            ...
            {people.length === 0 ? (
              ...
              <div className='space-y-2'>
                {people.map((person) => (
                  <div key={person.id} className='flex items-center justify-between p-2 bg-gray-50 dark:bg-gray-800 rounded'>
                    <div className='flex items-center space-x-2'>
                      <User size={16} className='text-gray-500' />
                      <div>
                        <span className='font-medium text-sm'>{person.name}</span>
                        <span className='text-xs text-gray-500 ml-2'>({person.birth_year})</span>
                      </div>
                    </div>
                    <div className='flex items-center space-x-1'>
                      <Button
                        size='sm'
                        variant='ghost'
                        onClick={() => setEditingRelationship({ personId: person.id, type: relationshipType as any })}
                      ...
                      <Button
                        ...
                        onClick={() => {
                          if (onModifyRelationship) {
                            onModifyRelationship(selectedPerson.id, person.id, 'disconnect', relationshipType as any);
                          } else {
                            console.warn('onModifyRelationship callback not provided');
                          }
                        }}
                        className='h-6 w-6 p-0 text-red-500 hover:text-red-700'
                        title={`Remove ${relationshipType.slice(0, -1)} relationship`}
                      >
                        <Unlink size={12} />
                      </Button>
                    </div>
            ...
            <Button
              size='sm'
              variant='outline'
              onClick={() => onAddRelative(selectedPerson.id, relationshipType as any)}
        ...
        <div className='space-y-2 max-h-48 overflow-y-auto'>
          {filteredSuggestions.length === 0 ? (
            ...
            filteredSuggestions.map((suggestion) => (
              <div key={suggestion.id} className={`p-2 rounded border hover:shadow-sm transition-all ${
                  ...
                  <div className='flex items-center space-x-2 flex-1 min-w-0'>
                    <div className='flex items-center space-x-1'>
                      ...
                      <div className={`p-1 rounded ${
                        suggestion.type === 'parent' ? 'bg-amber-100 text-amber-600 dark:bg-amber-900/20' :
                        suggestion.type === 'spouse' ? 'bg-pink-100 text-pink-600 dark:bg-pink-900/20' :
                        suggestion.type === 'child' ? 'bg-green-100 text-green-600 dark:bg-green-900/20' :
                    ...
                    <div className='flex-1 min-w-0'>
                      <p className='font-medium text-xs truncate'>{suggestion.label}</p>
                ...
                {suggestion.suggestedPersonId && (
                  <div className={`mt-2 p-1 rounded text-xs border-l-2 ${
                    ...
                    <p className="font-medium">{allData[suggestion.suggestedPersonId]?.name}</p>
                  </div>
                )}
              </div>
            ))
    ...
    <Card
      ...
      <div className='flex space-x-1 mb-4 p-1 bg-gray-200 dark:bg-gray-700 rounded-lg'>
        {[
          ...
          { id: "suggestions", label: "Suggestions", icon: <Sparkles size={14} /> },
